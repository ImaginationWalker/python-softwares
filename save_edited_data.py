...
def write(registration_no, current_day_date, student, class_selection_combo, date_of_birth, religion, radio, skills,
          ...
          form_four_certificate, registration_no_entry, from_four_certificate_entry, b_certificate_entry):
    try:
        registration_no_entry.config(state='normal')
        reg_number = registration_no.get()
        registration_no_entry.config(state='disabled')
        ...
        if (reg_number == '' or current_date == '' or student_name == '' or dob == '' or student_religion == ''
                or student_skills == '' or fathers_name == '' or mothers_name == '' or mothers_occupation == ''
                or student_class == '' or gender is None):
            ...
            sheet.cell(column=1, row=reg_number+1, value=reg_number)
            sheet.cell(column=2, row=reg_number+1, value=student_name)
            sheet.cell(column=3, row=reg_number+1, value=student_class)
            sheet.cell(column=4, row=reg_number+1, value=gender)
            sheet.cell(column=5, row=reg_number+1, value=dob)
            sheet.cell(column=6, row=reg_number+1, value=current_date)
            sheet.cell(column=7, row=reg_number+1, value=student_religion)
            sheet.cell(column=8, row=reg_number+1, value=student_skills)
            sheet.cell(column=9, row=reg_number+1, value=fathers_name)
            sheet.cell(column=10, row=reg_number+1, value=mothers_name)
            sheet.cell(column=11, row=reg_number+1, value=fathers_occupation)
            sheet.cell(column=12, row=reg_number+1, value=mothers_occupation)
            ...
            try:
                ...
                pass
            messagebox.showinfo('Success', 'Data was updated Successful!')


            if f_certificate and birth_certificate:
                ...
                subprocess.call(['cp', f_certificate, new_path])
                ...
                new_f_path = f'{new_path}/student_{str(reg_number)}_form_four_certificate.{old_f_extension}'
                ...
                new_f_path = f'{new_b_path}/student_{str(reg_number)}_birth_certificate.{old_f_extension}'
                os.rename(old_path, new_f_path)
